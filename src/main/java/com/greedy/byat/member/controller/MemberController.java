...
public class MemberController {
    ...
    * @ return : Service에서 반환 받은 이메일에 인증번호 전송한 결과가 담긴 값을 비동기방식으로 이용하기 위해 가공된 값을 return합니다. 
    * @ exception : NotexistEmailException(입력한 아이디가 존재하지 않는 경우에 대한 익셉션입니다.)
    */
    ...
    * @ return : Service에서 반환 받은 이메일에 인증번호  재전송한 결과가 담긴 값을 비동기방식으로 이용하기 위해 가공된 값을 return합니다. 
    * @ exception : NotexistEmailException(입력한 아이디가 존재하지 않는 경우에 대한 익셉션입니다.)
    */
    ...
    * @ param String confirmPassword : 비밀번호 변경을 위해 새로 입력한 비밀번호 확인값이 담긴 파라미터입니다.
    ...
    * @ param SessionStatus status : loginMember의 세션만료를 위한 파라미터입니다.
    * @ param RedirectAttributes rttr : addFlashAttribute로 메시지를 1회 출력하기 위함입니다.
    * @ return : /member/login으로 재요청을 하기 위한 return입니다.  
    */
	@GetMapping("/logout")
